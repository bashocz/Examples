...
int zakaznik(int socket, int index)
{	
	char zprava[128];
...
int main(int argc, char *argv[])
{
  ...
  {
    perror("Nelze vytvorit komunikacni pipe.\n");
    exit(1);
  }
	...
	if (klient_pid[klient_cnt] == 0) // pokracuje holic
	{
		int ret = holic(); // navratovy kod
		return ret;
	}
	 // jeden vedlejsi proces spusten (holic)
	klient_cnt++;
    ...
    FD_SET(sock_listen, &pro_in);
		...
		else if (FD_ISSET(sock_listen, &pro_in))
		{
			...
			if (klient_pid[klient_cnt] == 0) // pokracuje klient
			{
				...
				return zakaznik(sock_client, klient_cnt);
			}
