...
// datova struktura pro sdilenou pamet
struct shm_data
{
	int filozof[N];	// stavy jednotlivych filozofu ( PREMYSLI, HLADOVI, POJIDA )
	int zidle[N];		// stavy zidli ( VOLNA, OBSAZENA )
};

// globalni id sdilene pameti
int shm_data_id = -1;

// pro spolecne promene
shm_data *global_data = NULL;	// ukazatel na sdilenou pamet
...
{
	global_data->filozof[zidle] = PREMYSLI;
	global_data->zidle[zidle] = VOLNA;
}

int vytvor_pamet()
{
	shm_data_id = shmget(5556, sizeof(shm_data), 0666);
	
	if (shm_data_id < 0)
	{
		shm_data_id = shmget(5556, sizeof(shm_data), IPC_CREAT | 0666);
	
		if (shm_data_id < 0)
			return -1;
	}

	global_data = (shm_data *) shmat(shm_data_id, NULL, 0);
	if ((int) global_data == -1)
		return -2;
...
int uvolni_pamet()
{
  if (shm_data_id >= 0)
  {
		if (shmctl(shm_data_id, IPC_RMID, NULL) < 0)
			return 1;
	}
	shm_data_id  = -1;
	return 0;
...
void tiskVidlicek(int zidle)
{
	...
	for (int i = 0; i < N; i++)
	{
	  ...
	  {
			int vidlicka = (global_data->filozof[i] == POJIDA) ? 1 : 0;
			printf("%d", vidlicka);
		}
	}
	printf("}\n");
...
int filozof(int socket, int zidle)
{	
	char zprava[128];
